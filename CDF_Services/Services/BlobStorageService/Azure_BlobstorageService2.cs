...
{
    public class Azure_BlobstorageService2 : IAzure_BlobstorageService2
    {
        private readonly IConfiguration _configuration;

        public Azure_BlobstorageService2(IConfiguration configuration)
        {
        ...
        async Task<string> getBLOBSasUrlForJsonFormat(string blobName)
        {
            ...
            try
            {
                string accountName = _configuration["Azure-Blobstorage-2:AccountName"];
                string containerName = _configuration["Azure-Blobstorage-2:ContainerName"];
        ...
        public async Task<IActionResult> DeleteBlob(string fileName)
        {
            using (StreamWriter writer = System.IO.File.AppendText("log.txt"))
            {
                try
                {
                    ...
                    string storageAccountName = _configuration["Azure-Blobstorage-2:AccountName"];
            ...
            using (StreamWriter writer = System.IO.File.AppendText("log.txt"))
            {
                try
                {
                    ...
                    if (file.Length > 0)
                    {
                        ...
                        string storageAccountName = _configuration["Azure-Blobstorage-2:AccountName"];
                        string fileSystemName = _configuration["Azure-Blobstorage-2:ContainerName"]; // File system (container) name for Data Lake
