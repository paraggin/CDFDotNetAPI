...
{
    public class Azure_BlobstorageService3 : IAzure_BlobstorageService3
    {
        private readonly IConfiguration _configuration;

        public Azure_BlobstorageService3(IConfiguration configuration)
        {
        ...
        async Task<string> getBLOBSasUrlForJsonFormat(string blobName)
        {
            ...
            try
            {
                string accountName = _configuration["Azure-Blobstorage-3:AccountName"];
                string containerName = _configuration["Azure-Blobstorage-3:ContainerName"];
        ...
        public async Task<IActionResult> DeleteBlob(string fileName)
        {
            using (StreamWriter writer = System.IO.File.AppendText("log.txt"))
            {
                try
                {
                    ...
                    string storageAccountName = _configuration["Azure-Blobstorage-3:AccountName"];
            ...
            using (StreamWriter writer = System.IO.File.AppendText("log.txt"))
            {
                try
                {
                    ...
                    if (file.Length > 0)
                    {
                        ...
                        string storageAccountName = _configuration["Azure-Blobstorage-3:AccountName"];
                        string fileSystemName = _configuration["Azure-Blobstorage-3:ContainerName"]; // File system (container) name for Data Lake
